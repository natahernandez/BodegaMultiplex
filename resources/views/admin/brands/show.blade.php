            ...
            <div class="col-sm mb-2 mb-sm-0">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-no-gutter">
                        ...
                        <li class="breadcrumb-item"><a class="breadcrumb-link" href="{{ route('brands.index') }}">Marcas</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{ $brand->nombre }}</li>
                    </ol>
                </nav>

                <h1 class="page-header-title">{{ $brand->nombre }}</h1>
                <p class="page-header-text">Detalles de la marca</p>
            </div>
            
            <div class="col-sm-auto">
                <a class="btn btn-primary" href="{{ route('brands.edit', $brand) }}">
                    <i class="bi-pencil"></i> Editar Marca
                </a>
            </div>
        </div>
    </div>
        ...
        <div class="col-lg-8">
            ...
            <div class="card">
                ...
                <div class="card-header">
                    <h4 class="card-header-title">Información de la Marca</h4>
                </div>
                <!-- End Header -->
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-sm-3">
                            @if($brand->imagen_url)
                                <img src="{{ $brand->imagen_url }}" alt="{{ $brand->nombre }}" class="img-fluid rounded">
                            @else
                        ...
                        <div class="col-sm-9">
                            <div class="row">
                                <div class="col-sm-6">
                                    <dl class="row">
                                        ...
                                        <dd class="col-sm-8">{{ $brand->nombre }}</dd>
                                        ...
                                        <dd class="col-sm-8">{{ $brand->productos->count() }} productos</dd>
                                    </dl>
                                </div>
                                ...
                                <div class="col-sm-6">
                                    <dl class="row">
                                        ...
                                        <dd class="col-sm-7">{{ $brand->created_at->format('d/m/Y H:i') }}</dd>

                                        <dt class="col-sm-5">Última actualización:</dt>
                                        <dd class="col-sm-7">{{ $brand->updated_at->format('d/m/Y H:i') }}</dd>
                                    </dl>
                                </div>
                            </div>
                            ...
                            @if($brand->descripcion)
                                <div class="mt-3">
                                    <h6>Descripción:</h6>
                                    <p class="text-body">{{ $brand->descripcion }}</p>
                                </div>
                            @endif
                        </div>
                    </div>
            ...
            <div class="card">
                ...
                <div class="card-header card-header-content-between">
                    <h4 class="card-header-title">Productos de la Marca</h4>
                    <span class="badge bg-soft-secondary text-secondary">{{ $brand->productos->count() }} productos</span>
                </div>
                <!-- End Header -->
                ...
                <div class="table-responsive">
                    <table class="table table-borderless table-thead-bordered table-nowrap table-align-middle card-table">
                        ...
                        <tbody>
                            @foreach($brand->productos as $producto)
                            <tr>
                                <td>
                    ...
                    <div class="d-grid gap-2">
                        ...
                        <form action="{{ route('brands.toggleStatus', $brand) }}" method="POST" data-confirm="¿Cambiar el estado de esta marca?">
                            ...
                            <button type="submit" class="btn btn-{{ $brand->activo ? 'warning' : 'success' }} w-100">
                                @if($brand->activo)
                                    <i class="bi-pause me-1"></i> Desactivar Marca
                                @else
                                    <i class="bi-play me-1"></i> Activar Marca
                                @endif
                            </button>
                        </form>
                        ...
                        <div class="dropdown">
                            <button class="btn btn-danger dropdown-toggle w-100" type="button" id="dropdownMenuButton" data-bs-toggle="dropdown" aria-expanded="false">
                                <i class="bi-trash me-1"></i> Eliminar Marca
                            </button>
                                ...
                                <li>
                                    <form action="{{ route('brands.destroy', $brand) }}" method="POST" data-confirm="¿Eliminar esta marca de forma permanente?">
                                        ...
                                        <button type="submit" class="dropdown-item text-danger">
                                            <i class="bi-trash me-1"></i> Sí, eliminar marca
                                        </button>
                                    </form>
                                </li>
                            </ul>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-6">
                            <div class="text-center">
                                <span class="display-4 text-dark d-block">{{ $brand->productos->count() }}</span>
                                <span class="d-block">Productos</span>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="text-center">
                                <span class="display-4 text-dark d-block">{{ $brand->productos->where('activo', true)->count() }}</span>
                    ...
                    <div class="row">
                        <div class="col-6">
                            <div class="text-center">
                                <span class="display-4 text-dark d-block">{{ $brand->productos->sum('stock_actual') }}</span>
                                <span class="d-block">Stock Total</span>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="text-center">
                                <span class="display-4 text-dark d-block">${{ number_format($brand->productos->sum('precio_venta'), 2) }}</span>
